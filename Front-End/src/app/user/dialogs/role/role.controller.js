      ...
      function userAssignment(role, chip){
        ...
        userService.userAssignment(params).then(function(response){
          ...
          if (response.user.profileExists)
            response.user.avatar = window.appBaseUrl3 + '/images/agents/' + angular.lowercase(response.user.username) + '.jpg?timestamp=' + new Date().getTime();
          else
            response.user.avatar = '/efadminpanel/assets1/images/avatars/profile.jpg?timestamp=' + new Date().getTime();
